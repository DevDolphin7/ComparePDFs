...
class Data():
    def __init__(self,cleaning=False,pyc=True):
        # Set class variables
        self.packages = []
        self.package_path = []
        self.module_paths = []
    ...
    def define_dat_modules(self):
        ...
        for paths in self.package_path:
            self.all_module_paths = glob.glob(join(paths, "*.pyc"))
            if self.cleaning and not self.pyc: self.all_module_paths = glob.glob(join(paths, "*.py"))
            for module_path in self.all_module_paths:
                self.module_paths.append(module_path)
        file_ext = -4
        if self.cleaning and not self.pyc: file_ext = -3
